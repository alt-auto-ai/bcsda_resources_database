// analyze-txt-title.js
// Script to extract document titles from TXT files and write results to output.csv
...
async function main() {
  ...
  const tasks = rows.map((row, idx) => limit(async () => {
    ...
    if (!filePath) {
      row[titleColumnIndex] = 'Error_file_missing';
