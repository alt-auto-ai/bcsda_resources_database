// analyze-txt-summary.js
// Script to extract summary from TXT files and write results to output.csv
...
async function main() {
  ...
  let summaryColumnIndex = normalizedHeader.indexOf('summary');
  if (header.length === 0) {
  ...
  if (summaryColumnIndex === -1) {
    header.push('summary');
    summaryColumnIndex = header.length - 1;
  }
  rows.forEach((row) => {
    while (row.length < header.length) row.push('');
  });

  const rowsWithBlankSummary = rows.filter((row) => {
    const fileName = normalizeCell(row[fileColumnIndex]);
    return fileName && isBlankValue(row[summaryColumnIndex]);
  }).length;
  console.log(`Rows with blank summary to process: ${rowsWithBlankSummary}`);

  const limit = pLimit(MODEL_CONFIG.concurrency);
  const tasks = rows.map((row, idx) => limit(async () => {
    const fileName = normalizeCell(row[fileColumnIndex]);
    if (!fileName) return;
    if (!isBlankValue(row[summaryColumnIndex])) return;

    const filePath = resolveTxtPath(fileName);
    if (!filePath) {
      row[summaryColumnIndex] = 'Error_file_missing';
    ...
    try {
      ...
      let summary = normalizeModelAnswer(response.output_text || '');
      if (!summary) summary = 'Unknown';
      row[summaryColumnIndex] = summary;
      console.log(`  Model response: ${summary}`);
    } catch (err) {
      console.error(`  Error processing ${fileName}:`, err.message || err);
      row[summaryColumnIndex] = `Error_${formatErrorMessage(err)}`;
    }
  }));
