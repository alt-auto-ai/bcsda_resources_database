// analyze-txt-year.js
// Script to extract year values from TXT files and write results to output.csv
...
async function main() {
  ...
  // Read output.csv and preserve all existing rows/columns
  ...
  if (detectedFileColumnIndex === -1) {
    console.error(`Missing 'file' column in CSV header: ${header.join(',')}`);
    process.exit(1);
  }
  const fileColumnIndex = detectedFileColumnIndex;
  ...
  const rowsWithBlankYear = rows.filter((row) => {
    const fileName = normalizeCell(row[fileColumnIndex]);
    return fileName && isBlankValue(row[responseColumnIndex]);
  }).length;
  console.log(`Rows with blank ${RESPONSE_COLUMN} to process: ${rowsWithBlankYear}`);
  if (rowsWithBlankYear === 0) {
  ...
  const tasks = rows.map((row, idx) => limit(async () => {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        console.error(`  Error processing ${fileName} (attempt ${attempt}/3):`, errorMessage);
        if (attempt === 3) {
          row[responseColumnIndex] = `Error_${errorMessage}`;
          updatesCount++;
        }
      }
    }
