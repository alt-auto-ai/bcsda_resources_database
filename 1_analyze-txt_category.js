// analyze-txt.js
// Script to analyze TXT files in txt_inputs using OpenAI Responses API models
...
function resolveTxtPath(fileName) {
  const directPath = path.join(INPUT_DIR, fileName);
...
async function main() {
  ...
  console.log('--- TXT Category Analysis Script ---');
  ...
  const tasks = csvRows.map((row, idx) => limit(async () => {
    ...
    if (!filePath) {
      const missingMessage = `File not found in txt_inputs: ${fileName}`;
      row[responseColumnIndex] = `Error_${missingMessage}`;
      console.error(`\n[${idx + 1}/${csvRows.length}] ${missingMessage}`);
      return;
    }
          ...
          input: [
            {
              ...
              content: [
                {
                  type: 'input_text',
                  text: `${prompt}\n\nDocument name: ${path.basename(filePath)}\n\nDocument text:\n${txtContent}`,
                },
              ],
            },
          ],
